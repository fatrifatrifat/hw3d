	...
	TestObject(d3dApp, rng, adist, ddist, odist, rdist)
{
	...
	if (!IsStaticInitialized())
	{
		...
		AddStaticBind(std::make_unique<VertexBuffer>(d3dApp, vbuf));
		...
		AddStaticBind(std::make_unique<InputLayout>(d3dApp, vbuf.GetLayout().GetD3DLayout(), pvsbc));
		...
		pmc.color = material;
		AddStaticBind(std::make_unique<PixelConstantBuffer<PSMaterialConstant>>(d3dApp, pmc, 1u));
	}
	else
	{
		SetIndexFromStatic();
