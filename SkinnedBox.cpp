...
SkinnedBox::SkinnedBox(D3DApp& d3dApp,
	std::mt19937& rng,
	...
	phi(adist(rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		AddStaticBind(std::make_unique<Texture>(d3dApp, Surface::FromFile("Images\\cube.png")));

		auto pvs = std::make_unique<VertexShader>(d3dApp, L"TextureVS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(d3dApp, L"TexturePS.cso"));
