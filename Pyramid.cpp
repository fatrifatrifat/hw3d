...
Pyramid::Pyramid(D3DApp& d3dApp, std::mt19937& rng,
	...
	TestObject(d3dApp, rng, adist, ddist, odist, rdist)
{
	...
	{
		auto pvs = std::make_unique<VertexShader>(d3dApp, L"BlendedPhongVS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(d3dApp, L"BlendedPhongPS.cso"));
