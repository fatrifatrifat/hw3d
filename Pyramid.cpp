	...
	phi(adist(rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		auto pvs = std::make_unique<VertexShader>(d3dApp, L"ColorBlendVS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(d3dApp, L"ColorBlendPS.cso"));
